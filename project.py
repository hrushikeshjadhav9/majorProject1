...
def trainModel(config):
    ...
    #Rewards per intersection per epoch
    epoch_rewards = {id_:[] for id_ in intersection_id}
    #Scores per intersection per epoch
    epoch_scores = {id_:[] for id_ in intersection_id}
    ...
    with tqdm(total=config["epochs"] * config["num_step"]) as pbar:
        for epoch in range(config['epochs']):
            ...
            epoch_reward = {id_:0 for id_ in intersection_id} 
            epoch_score = {id_:0 for id_ in intersection_id} 
            epoch_length = 0
            ...
            while epoch_length < config['num_step']:
                ...
                for i in range(config['phase_step'] - 1):
                ...
                pbar.set_description("t_st:{}, epoch:{}, st:{}, r:{}".format(total_step, epoch+1, epoch_length, print_reward))
            ...
            if (epoch + 1) % config['save_freq'] == 0:
                Agent.save(config['model_out'] + "/{}_{}.h5".format(config['algo'], epoch+1))
